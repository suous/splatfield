...
impl RenderScratch {
    ...
    /// last frame's emission. Returns the tile-grid bounds.
    fn prepare(
        &mut self,
        ...
        img_size.map(|c| c.div_ceil(layout::TILE_WIDTH))
    }
}
...
impl Splats {
    ...
    /// cubecl's blocking reads poll once and panic on wasm; native callers go
    ...
    pub async fn render_with_async(
        ...
        let tile_bounds = scratch.prepare(client, total, img_size);
        ...
        // The frame's only GPU sync. Everything below it needs the emitted
        // counts, so the readback cannot move off this path.
        ...
        let tile_bits = bits_for(tile_bounds.x * tile_bounds.y);
    ...
    /// Blocking [`Self::render_with_async`] for native callers: drives the
