...
impl RadixScratch {
    ...
    pub fn argsort(
        ...
        // (Tile sorts on typical grids have bits <= 8, where both paths take
        // the same pass count and plane would pay the premium for nothing.)
        let planes = plane_sort_path(&keys.client)
            .filter(|_| bits.div_ceil(BITS_PLANE) * 4 <= bits.div_ceil(4) * 3);
        radix_argsort_path(keys, vals, n, bits, write_keys, planes, self)
    }
}
